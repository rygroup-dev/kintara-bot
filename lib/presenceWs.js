...
class Presence extends EventEmitter {
  constructor(shard = config.shard) {
    ...
    this.eq = null;            // equipped item type, e.g. wild_sword for combat
    ...
    this._mobsAt = 0;          // timestamp of the latest mob snap
    this.pendingWblk = null;   // null = no manifest pending; [] is a valid empty manifest
  }
  ...
  _sendPos(full) {
    ...
    // wild realm: HP + shield + spawn-protect + blocked-tile manifest for mob spawn/pathing.
    if (/^wild/.test(this.region)) {
      base.php = Math.max(0, Math.min(100, this.hp | 0));
      base.wsh = Math.max(0, Math.min(5, this.shield | 0));
      base.wsp = 0;
      if (this.pendingWblk !== null) { base.wblk = this.pendingWblk; this.pendingWblk = null; }
    }
    // wild blocked-tile manifest non-wild path (legacy harvest hosting)
    else if (this.pendingWblk !== null) { base.wblk = this.pendingWblk; this.pendingWblk = null; }
...
// ===== COMBAT (Wilderness) =====
  // Server-authoritative mobs: the hub broadcasts position+HP in snap.npcs.wildMobs.
  // Coord wild: world x=col-24.5, z=row-24.5 (WILD 50x50, off=-24.5).
  static WILD_OFF = -24.5;

  /** Track life epoch from server messages and echo it back in pos+wm_ev. */
  _trackLifeEpoch(d) {
    const le = d && d.le != null && Number.isFinite(Number(d.le)) ? Number(d.le) | 0 : null;
    if (le != null && le > (this.lifeEpoch | 0)) this.lifeEpoch = le;
  }

  /** Parse snap: extract wildMobs nested under npcs and self HP. */
  _onSnap(d) {
    // Self HP from players[] self-entry when available (php).
    ...
    // wildMobs is nested under npcs, not top-level.
  ...
  /** wild_mb_ack: hub replies with HP/shield after contact reports. We still update HP
   *  if the server pushes it as a defense-in-depth safety path. */
  _onWildMbAck(d) {
  ...
  /** pvit broadcast is usually remote; if pid is us, consume HP/shield from it. */
  _onPvit(d) {
  ...
  /** Equip item and broadcast eq in pos. 'wild_sword' is required for wm_ev. */
  equip(itemType) { this.eq = itemType || null; this._sendPos(false); }

  /** Current Wilderness tile from current position. */
  wildTile() { return { col: Math.round(this.pos.x - Presence.WILD_OFF), row: Math.round(this.pos.z - Presence.WILD_OFF) }; }

  /** Nearest live mob from current position, using Chebyshev tile distance. */
  nearestMob() {
  ...
  _sendPosMoving() {
    ...
    if (/^wild/.test(this.region)) {
      ...
      m.wsp = 0;
      if (this.pendingWblk !== null) { m.wblk = this.pendingWblk; this.pendingWblk = null; }
    }
